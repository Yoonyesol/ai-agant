...
const Preview = () => {
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center">
      ...
      <div className="w-full h-full min-h-screen bg-slate-50 relative max-w-md shadow-2xl flex flex-col overflow-hidden">
        
        {/* Header - Sticky within the mobile frame */}
        <div className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm flex items-center justify-between p-4 safe-area-top">
          <button onClick={() => navigate('/')} className="p-2 text-slate-600 hover:bg-slate-100 rounded-full">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <span className="font-bold text-slate-800">계약서 미리보기</span>
          <div className="w-10"></div>
        </div>

        {/* PDF Viewer Area */}
        <div 
          className="flex-1 overflow-auto flex items-start justify-center pt-8 pb-32 px-4"
          ref={(el) => {
        ...
        <div className="absolute bottom-0 left-0 right-0 z-50 pointer-events-none">
          ...
          <div className="bg-white border-t border-slate-200 p-4 pb-8 safe-area-bottom shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] pointer-events-auto">
              <button
